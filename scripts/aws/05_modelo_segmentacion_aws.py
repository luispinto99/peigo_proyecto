"""
glue_job_modelo_segmentacion.py
...
# ---------------------------------------------------------------------------
args = getResolvedOptions(sys.argv, ["bucket", "for_modelling_prefix", "models_prefix"])
BUCKET = args["bucket"]
FOR_MODELLING_PREFIX = args.get("for_modelling_prefix", "data/for_modelling")
MODELS_PREFIX = args.get("models_prefix", "models")
 
s3 = boto3.client("s3")
 
 
def ruta_for_modelling(tabla: str) -> str:
    return f"s3://{BUCKET}/{FOR_MODELLING_PREFIX}/{tabla}/{tabla}.parquet"
 
 
base = pd.read_parquet(ruta_for_modelling("base_segmentacion"))
...
# ---------------------------------------------------------------------------
# 2. Elegir k probando varias opciones (silhouette score)
# ---------------------------------------------------------------------------
resultados_k = []
for k in [3, 4, 5, 6]:
...
# ---------------------------------------------------------------------------
# 3. Entrenar el modelo final con el k elegido
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Guardar modelo (pickle -> S3 vía buffer en memoria) y resultado final
# ---------------------------------------------------------------------------
artefactos = {
    "kmeans": kmeans,
...
buffer = io.BytesIO()
pickle.dump(artefactos, buffer)
key_modelo = f"{MODELS_PREFIX}/modelo_segmentacion.pkl"
s3.put_object(Bucket=BUCKET, Key=key_modelo, Body=buffer.getvalue())
print(f"\nModelo y mapeo guardados en: s3://{BUCKET}/{key_modelo}")
...
destino = ruta_for_modelling("clientes_segmentados")
